let three = function(sketch) { 
    ...
    let convertion = function(min) {
        ...
        //     let letters = [min.listTransitions()[i].letter];

        //     for (let j = i+1; j < min.listTransitions().length-1; j++) {
        //     if (min.listTransitions()[i].initial === min.listTransitions()[j].initial &&
        //         min.listTransitions()[i].final === min.listTransitions()[j].final) {
        //         letters.push(min.listTransitions()[j].letter)
        //         }
        //     }

        //     if (min.listTransitions()[i].initial == min.listTransitions()[i].final) {
        //     loops.push(new Loop(nodes.find( (elem) => {
        //         return elem.name == min.listTransitions()[i].initial;
        //     }), letters));
        //     } else {
        //         // console.log(nodes.find( (elem) => {
        //         //     return elem.name == min.listTransitions()[i].initial;
        //         // }), nodes.find( (elem) => {
        //         //     return elem.name == min.listTransitions()[i].final;
        //         // }), letters)
        //     transitions.push(new Transition(nodes.find( (elem) => {
        //         return elem.name === min.listTransitions()[i].initial;
        //     }), nodes.find( (elem) => {
        //         // console.log(min.listTransitions()[i])
